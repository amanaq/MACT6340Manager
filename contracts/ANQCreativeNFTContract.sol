...
error ANQCreativeNFTContract__MaxSupplyReached();
error ANQCreativeNFTContract__ValueNotEqualPrice();
error ANQCreativeNFTContract__WrongAvenueForThisTransaction();

/// @custom:security-contact amana@amanaqureshi.com
contract ANQCreativeNFTContract is
    ERC721,
    ERC721Enumerable,
    ERC721URIStorage,
    ERC721Burnable,
    ...
    ReentrancyGuard
{
    ...
    ) ERC721("ANQCreativeNFTContract", "ANQ") Ownable(msg.sender) {
    ...
    receive() external payable {
        revert ANQCreativeNFTContract__WrongAvenueForThisTransaction();
    }
    ...
    function mintTo(
        ...
        if (tokenId >= i_max_tokens) {
            revert ANQCreativeNFTContract__MaxSupplyReached();
        }
        // make sure there us the right amount of money
        if (msg.value != i_mint_price) {
            revert ANQCreativeNFTContract__ValueNotEqualPrice();
        }
        _tokenIdCounter++;
